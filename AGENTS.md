...
*   **Deployment**: Vercel
*   **AI Integration**: Gemini API via Vercel AI SDK using `@ai-sdk/google`
...
2.  **`google-gemini-3-5-flash`** (Product Slug representing Google Gemini 3.5 Flash)
...
> *   The actual provider `model_id` (e.g., `gemini-1.5-flash` or `gemini-2.0-flash-exp`) must be resolved from database/static seed configuration in the backend.
...
> *   **Do not use** Gemini Interactions API or raw provider-specific call layers in the MVP.
...
const result = await generateText({
  model: google('gemini-1.5-flash'), // model id resolved dynamically
...
*   **Two Model Profiles**: `general-llm` and `google-gemini-3-5-flash`.
