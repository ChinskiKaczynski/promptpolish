...
*   **Deployment**: Vercel
*   **AI Integration**: OpenRouter API via Vercel AI SDK using `@openrouter/ai-sdk-provider` (with target model `deepseek/deepseek-v4-flash`)
...
> *   The actual provider `model_id` (e.g., `deepseek/deepseek-v4-flash`) must be resolved from database/static seed configuration in the backend.
...
const result = await generateText({
  model: openrouter.chat(process.env.OPENROUTER_MODEL_ID || 'deepseek/deepseek-v4-flash'),
