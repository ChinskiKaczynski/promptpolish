...
1.  **`general-llm`**
    - _Purpose_: A balanced general-purpose assessment model.
    - _Default Target_: Resolves to the current industry-standard general analysis model in the database config.
2.  **`openrouter-deepseek-v4-flash`**
    - _Purpose_: Specifically calibrated for prompt testing under DeepSeek-family models via OpenRouter.
    - _Default Target_: Resolves to the verified active OpenRouter model ID.
...
interface ModelProfile {
  ...
  model_id: string; // Provider model name (e.g. 'openrouter/owl-alpha')
