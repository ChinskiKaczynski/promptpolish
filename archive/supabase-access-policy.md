...
# Supabase Access Control & Security Policy â€” PromptPolish
...
User requests /result/[id]
   â”śâ”€â”€ 1. Server Component intercepts request.
   â”śâ”€â”€ 2. Extracts `owner_anonymous_id` from secure httpOnly cookie.
   â”śâ”€â”€ 3. Queries Supabase using:
   â”‚      SELECT * FROM prompt_analyses 
   â”‚      WHERE id = [id] AND owner_anonymous_id = [owner_anonymous_id]
   â”śâ”€â”€ 4. If record found: Render page.
   â”śâ”€â”€ 5. If record missing (wrong ID): Return 404.
   â””â”€â”€ 6. If record exists but owner UUID does not match: Return 403 Forbidden.
```
