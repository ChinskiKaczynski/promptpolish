...
| API — not authenticated | `401 Unauthorized` JSON |
| API — non-admin | `403 Forbidden` JSON |
...
| `failure_rate` | `failed / started × 100` |
...
| `copy_rate` | `copies / completed × 100` |
...
| `returning_owners_count` | Owners who completed ≥2 analyses |
| `returning_rate` | `returning / owners_with_completed × 100` |
| `median_completed_analyses_per_owner` | Median of per-owner completion counts |
| `owner_usage_buckets` | Distribution: 1 / 2–3 / 4–10 / 10+ analyses |
...
### 5. Reliability
Failures and errors — **raw error messages are never returned**.
...
### 6. Limits
Rate-limiting signals — **no owner IDs returned**.

| Metric | Description |
|---|---|
| `limit_reached` | Count of `limit_reached` events |
| `limit_reached_rate` | `limit_reached / unique_active_owners × 100` |
...
Detection and blocking — **no secret values are returned**.
...
Aggregate-only — **no prompt text returned**.

- Score distributions across 5 buckets (weak → excellent)
...
| Signal | Logic |
|---|---|
| `copy_rate_status` | `strong` ≥40%, `acceptable` 20–39%, `weak` <20% |
| `feedback_status` | `strong` if ≥70% positive and ≥3 feedback |
| `retention_status` | `strong` if ≥25% returning (min 5 owners) |
| `reliability_status` | `strong` <5% fail, `warning` 5–15%, `blocking` >15% |
