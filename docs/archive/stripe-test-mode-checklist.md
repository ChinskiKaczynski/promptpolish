...
# Stripe Billing Test-Mode Verification Checklist

This guide provides a manual verification checklist to safely test the Stripe integration in development/test-mode before any production deployment.
...
2. Register/Log in to a new test account at `http://localhost:3000/login`.
...
6. Once redirected back to `http://localhost:3000/pricing?upgrade=success`, verify:
...
3. Verify the local account page displays the **ZalegĹ‚oĹ›Ä‡ w pĹ‚atnoĹ›ci (Grace Period)** warning banner.
...
4. Verify that the **DostÄ™p zawieszony (Access Suspended)** message is displayed.
...
Verify that the admin metrics page (`/admin/metrics`) reflects the proper event counts:
- `checkout_started` (when clicking the Stripe checkout button)
- `checkout_completed` (upon webhook session completion)
- `subscription_activated` (upon active webhook event)
- `subscription_canceled` (upon cancellation webhook event)
- `checkout_failed` (on payment failure events)

---

## 6. Rollback / Cleanup Steps
1. Delete the test customer in the Stripe Dashboard to clean up test data.
2. Set `STRIPE_ENABLED=false` in `.env.local` to return your workspace to the default safe simulated Pro state.
