# Pre-Production Launch & Readiness Checklist — PromptPolish

> [!WARNING]
> **LEGAL & TAX BLOCKER — APP IS NOT LEGALLY READY FOR PRODUCTION**  
...
- [docs/stripe-test-mode-checklist.md](./stripe-test-mode-checklist.md) — Stripe test-mode setup and verification scenarios.
- [docs/legal-readiness.md](./legal-readiness.md) — GDPR, ePrivacy, and legal entity transition roadmap.
- [docs/billing-policy-draft.md](./billing-policy-draft.md) — Terms for subscriptions, cancellations, and grace periods.
- [docs/privacy-policy-draft.md](./privacy-policy-draft.md) — Personal data processing, sub-processors, and retention schedules.
- [docs/terms-of-service-draft.md](./terms-of-service-draft.md) — Core terms of service, AI disclaimers, and user obligations.
- [docs/launch-checklist.md](./launch-checklist.md) — Original MVP visual and layout quality gates.
- [docs/production-smoke-test.md](./production-smoke-test.md) — Live system verification steps post-deploy.
- [docs/operations-runbook.md](./operations-runbook.md) — Operations logging, rotations, database retention, and playbooks.
...
- [ ] **Monitoring & Runbook Log Signatures**: Confirm operations logging utilizes the correct signatures defined in [docs/operations-runbook.md](./operations-runbook.md):
  - `[PROVIDER_ERROR]` — Critical OpenRouter failures.
  - `[STRIPE_WEBHOOK_FAILURE]` — Database sync or webhook validation issues.
  - `[Sensitive Data Blocked]` — Preflight blocks on passwords, keys, or secrets.
...
- [ ] **Grace Period (past_due) Verification**:
  ...
  - Verify the account page shows the **Zaległość w płatności (Grace Period)** warning.
...
- [ ] **Downgrade Enforcement (unpaid/canceled)**:
  ...
  - Confirm the account page displays the **Dostęp zawieszony (Access Suspended)** message.
...
- [ ] **Account Deletion Flow Tested**: Validate that clicking "Usuń konto" triggers cascading deletes in Supabase (deleting profiles, history, and usage logs) and triggers a subscription cancellation webhook to Stripe.
