...
1. **Primary Model**: The engine attempts the request with the primary model resolved from the database catalog or `OPENROUTER_MODEL_ID` (defaults to `openrouter/owl-alpha`).
