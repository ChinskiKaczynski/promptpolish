# Final Manual Smoke-Test Checklist — PromptPolish v1 Private Beta

**Date:** 2026-06-13
**Author:** Senior QA & Release Auditor
**Status:** FINAL — Use this checklist for all post-deploy verification runs
...
> [!NOTE]
> **Preparation**: Have three browser profiles ready: (1) Owner profile, (2) Incognito/Private — unauthenticated, (3) Logged-in Free account. Use the Stripe CLI for billing tests.
...
- [ ] Submit a clean prompt (`"Napisz zwięzły opis produktu SaaS dla menedżerów"` — 52 chars).
...
### 2.4 Favorites
- [ ] Toggle the star ★ on a history item.
...
- [ ] Verify Polish characters (`ąęćłńóśźż`) render correctly in the PDF.
...
### 3.5 Billing Page — STRIPE_ENABLED=false
...
> [!WARNING]
> Run Sections 4.2–4.5 **only on staging** with `STRIPE_ENABLED=true` pointing to Stripe test keys. Never run on production with live keys.
...
- [ ] Same for `/privacy`.
- [ ] `/result/[id]` — verify it does NOT serve a cached version to non-owners.
...
### 5.4 Public Share — No Private Controls
- [ ] See Section 1.7 — already covers this.
...
- [ ] Run `pnpm test` — `tests/api/production-readiness.test.ts` checks for secret exposure patterns.
- [ ] Inspect `.next/static/` — no secret key patterns in JS bundle files.

---

## Section 6: Server Observability

### 6.1 RLS — No Permission Denied Errors
...
| No secrets in logs/bundle | Sections 5.6–5.7 | _TBD_ | |
...
*   [v1-production-smoke-test-plan.md](./v1-production-smoke-test-plan.md) — Detailed Phase A/B/C/D scenarios (historical, Stripe staging)
*   [stripe-test-mode-checklist.md](./stripe-test-mode-checklist.md) — Stripe sandbox scenario setup
*   [operator-runbook.md](./operator-runbook.md) — Incident response and rollback procedures
*   [v1-release-notes.md](./v1-release-notes.md) — Current release state and known limitations
