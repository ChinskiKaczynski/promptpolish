# PromptPolish — kompletny plan wykonawczy v1.2

**Data:** 2026-05-23  
**Status:** plan wykonawczy do implementacji po poprawkach krytycznych  
...
**Źródło bazowe:** scalony plan v1.2 zaktualizowany o poprawki krytyczne: share disable, retencja, pełny Gemini smoke test, bundle audit, cost benchmark i share privacy tests  
...
## 1. Co zmienia wersja 1.1 i 1.2 względem planu bazowego
...
### 1.1. Poprawki v1.2 względem v1.1

Wersja 1.2 dodaje poprawki krytyczne wymagane przed publiczną betą:

```text
1. POST /api/share/disable jako obowiązkowy element MVP, nie opcjonalny.
2. Realny mechanizm retencji danych, np. scheduled cleanup/manual cleanup script.
3. Smoke test Gemini structured output na pełnym production-like analysisSchema.
4. UX warning: anonimowy wynik zależy od cookie i może być niedostępny po zmianie przeglądarki albo czyszczeniu cookies.
5. Cost benchmark przed betą: token usage, koszt analizy, provider errors, retry count, invalid schema count.
6. Jasna decyzja Gemini: generateContent-compatible flow przez AI SDK w MVP; Interactions API dopiero przy agentic/background workflows.
...
10. Dokument prawny jako draft: GDPR, cookies i vendor/data processing do review przed publicznym ruchem z UE.
```
...
- publiczny share link jako element MVP, ale z obowiązkowym disable endpointem,
...
Nie używaj Interactions API w MVP. Interactions API traktuj jako osobną decyzję architektoniczną do rozważenia dopiero przy agentic/background workflows, narzędziach wieloetapowych albo trwałych sesjach wymagających nowego kontraktu API.

[Wniosek] MVP ma pojedynczy synchroniczny flow analizy promptu, dlatego wystarczy generateContent-compatible flow przez AI SDK.
...
| `POST /api/share/disable` | Tak | Obowiązkowe wyłączenie publicznego share linku. |
| Share privacy tests | Tak | Publiczny link nie może ujawniać ownera, UUID, debug fields ani sensitive findings. |
| Retention cleanup | Tak | Polityka retencji musi mieć realny mechanizm wykonawczy. |
| Full-schema Gemini smoke test | Tak | Structured output musi przejść na produkcyjnym `analysisSchema`, nie tylko mini schema. |
| Bundle leak test | Tak | Krytyczny test braku sekretów w client bundle, sourcemapach, response body i logach. |
| Cost benchmark przed betą | Tak | Kontrola kosztu analizy, token usage i błędów providera. |
| Static check client-side Supabase reads | Tak | Prywatne `prompt_analyses` nie mogą być czytane client-side. |
| Legal readiness draft dla UE | Tak | GDPR/cookies/vendor processing jako draft do review przed publicznym ruchem z UE. |
...
- eksportów PDF/Markdown w pierwszym MVP.
```

[Wniosek] `/pricing`, eksport PDF i eksport Markdown usuń z pierwszego MVP. Pricing ma sens dopiero, gdy masz dane o copy rate, powrotach użytkowników i realnym koszcie analizy.
...
## 5A. Threat model MVP — top ryzyka
...
| Publiczny dostęp do prywatnego wyniku | Krytyczny | Średnie | `/result/[id]` owner-only, `/share/[token]` token only opt-in, server-side checks. |
...
## 5B. Decision ledger po korekcie v1.2
...
### 10.9. `docs/legal-readiness.md`

Ma zawierać:

```text
- status: draft, wymaga review przed publicznym ruchem z UE,
- pytania GDPR do weryfikacji,
- decyzję cookie/cookie banner,
- listę vendorów/procesorów danych: Supabase, Vercel, Google/Gemini i ewentualne narzędzia monitoringowe,
- zakres przetwarzanych danych,
- retencję danych,
- data deletion request flow jako draft,
- zakaz twierdzeń typu “GDPR compliant” bez review.
```

### 10.10. `docs/decision-log.md`
...
Anonymous result UX warning:

```text
Wynik prywatny jest powiązany z anonimową sesją w tej przeglądarce. Po zmianie przeglądarki, urządzenia albo wyczyszczeniu cookies dostęp do prywatnego wyniku może być niedostępny. Użyj publicznego linku share tylko wtedy, gdy świadomie chcesz udostępnić wynik.
```
...
# Retention
ANONYMOUS_ANALYSES_RETENTION_DAYS=30
USAGE_EVENTS_RETENTION_DAYS=90
FEEDBACK_EVENTS_RETENTION_DAYS=180
RETENTION_CLEANUP_DRY_RUN=false

# Cost benchmark / provider telemetry
ENABLE_AI_COST_BENCHMARK=true
...
### 17.5. `POST /api/share/disable` — obowiązkowe w MVP

Jeśli dodajesz share link w pierwszym MVP, musisz dodać możliwość wyłączenia linku w tej samej fazie.
...
- log usage_event: share_link_disabled,
- return success.
```

[Wniosek] Minimum wymagane to opt-in create + disable. Publiczny share bez disable endpointu nie przechodzi launch checklisty.
...
### 20.4. Mechanizm wykonawczy retencji

Polityka retencji musi mieć realny mechanizm wykonawczy przed publiczną betą:

```text
- skrypt/manual command do cleanupu lokalnie i preview,
- opcjonalnie Vercel Cron albo Supabase scheduled job dla production,
- dry-run mode, jeśli praktyczne,
- test: stare rekordy są usuwane/anonymizowane,
- test: świeże rekordy zostają,
- test: aktywne shared results nie są usuwane, jeśli polityka produktu mówi, że mają pozostać aktywne,
- high-risk blocked prompts nigdy nie są zapisywane raw.
```

[Wniosek] Jeśli shared result ma pozostać publiczny dłużej niż 30 dni, decyzję trzeba zapisać w `docs/retention-policy.md` i `docs/decision-log.md`.
...
- full-schema Gemini structured output smoke test,
- bundle leak test,
- static/code review check: prywatne `prompt_analyses` nie są czytane client-side,
- share privacy snapshot test,
...
- publiczny share nie ujawnia ownera, UUID, debug fields ani sensitive findings,
- pełny `analysisSchema` przechodzi smoke test Gemini,
- bundle scan nie znajduje sekretów w client output,
- retention cleanup działa zgodnie z polityką.
```
...
18. Test braku sekretów w sourcemapach, response body i logach.
19. Test share disable.
20. Test share privacy.
21. Test retention cleanup dry-run.
22. Test cost benchmark telemetry.
23. Test mobile layout.
24. Legal readiness draft: GDPR/cookies/vendor processing do review.
25. Deploy production dopiero po ręcznym smoke teście.
```
...
### Task 17A — secret and bundle exposure audit
...
### Task 14A — mandatory share disable and privacy tests
...
### Task 19A — full-schema Gemini smoke test and cost benchmark
...
- record token usage where provider/SDK exposes it.
- record approximate cost per analysis where usage metadata is reliable.
- record provider errors, retry count and invalid schema count.
...
[ ] /api/share/disable działa i wyłącza publiczny link.
[ ] Publiczny share nie pokazuje owner_anonymous_id, user_id, internal UUID, debug fields ani sensitive findings.
...
[ ] Retention cleanup działa albo jest gotowy do uruchomienia/schedule.
[ ] Bundle leak test nie znajduje sekretów w client bundle, sourcemapach, response body ani logach.
...
[ ] Full-schema Gemini smoke test wykonany albo jawnie oznaczony jako niewykonany z powodu braku klucza.
[ ] Cost benchmark przed betą zapisany.
[ ] Legal readiness draft zawiera GDPR/cookies/vendor processing do review.
...
14. Dodaj share link.
15. Dodaj obowiązkowy share disable + share privacy tests.
16. Dodaj anonymous limit.
17. Dodaj feedback i copy event.
18. Dodaj retention cleanup.
19. Dodaj privacy/terms/legal readiness draft.
20. Dodaj secret/bundle exposure audit.
21. Dodaj testy.
22. Dodaj AI eval fixtures.
23. Wykonaj AI quality eval pass.
24. Wykonaj full-schema Gemini smoke test i cost benchmark.
25. Zrób Vercel preview.
26. Smoke test.
27. Production readiness review.
28. Deploy production.
29. Mierz copy rate, feedback, share usage, sensitive-data events, provider errors, invalid schema count i koszt analizy.
30. Decyduj, czy rozwijać dalej.
...
- Publiczny /share/[token] zostaje w MVP jako opt-in z obowiązkowym disable endpointem.
...
- Retencja, bundle leak test, share privacy tests i full-schema Gemini smoke test są wymagane przed betą.
