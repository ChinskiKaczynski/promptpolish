...
describe('Owner Model Catalog Aliases', () => {
  ...
  it('rejects/falls back on invalid alias to GEMINI_MODEL_ID or default', () => {
    process.env.AI_MODEL_ALIAS = 'invalid-alias-name'
    process.env.GEMINI_MODEL_ID = 'gemini-2.5-flash'

    expect(getOwnerConfiguredModelId()).toBe('gemini-2.5-flash')
  })
  ...
  it('uses GEMINI_MODEL_ID directly if AI_MODEL_ALIAS is not specified', () => {
    process.env.GEMINI_MODEL_ID = 'google/gemini-2.0-flash-exp'
