...
`
  ...
  console.log('Running test analyzePrompt with length', longPrompt.length)
  try {
    ...
    console.log('Success! Overall score:', result.scores.overallScore)
  } catch (err) {
    const error = err as Error & { cause?: { name?: string; message?: string; error?: { issues?: unknown[] }; text?: string } }
    console.error('Failed with error:', error.message)
    if (error.cause) {
      console.error('Error name:', error.cause.name)
      console.error('Error message:', error.cause.message)
      if (error.cause.error) {
        console.error('Zod Error Issues:', JSON.stringify(error.cause.error.issues, null, 2))
      }
      if (error.cause.text) {
        console.log('Raw text from LLM:\n', error.cause.text)
      }
    }
    throw err
  }
