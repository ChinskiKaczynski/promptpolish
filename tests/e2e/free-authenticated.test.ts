...
test.describe('Free Authenticated Flows', () => {
  ...
  test('Free: dashboard shows Free plan and session persists', async ({ page }) => {
    await page.goto('/account')
    await expect(page.locator('text=Twój plan')).toBeVisible()
    ...
    // history is available and does not contain Pro user's prompt
    await page.goto('/history')
    await expect(page.locator('h1, h2')).toContainText(/Historia/i)
    await expect(page.locator('body')).not.toContainText('uzytkownika Pro')
  })

  test('Free: PDF shows gate/upgrade and does not download', async ({ page }) => {
    // Navigate to owned Free result
    await page.goto(`/result/${freeAnalysisId}`)
    await expect(page.locator('h1')).toContainText('Raport audytu promptu')

    // click PDF export
    await page.locator('text=Eksportuj PDF').click()
    // verify upgrade modal is shown
    await expect(page.locator('text=Odblokuj Eksport PDF')).toBeVisible()
  })

  test('Isolation: Free cannot open Pro result', async ({ page }) => {
    await page.goto(`/result/${proAnalysisId}`)
    // Expect 404 page
    await expect(page.locator('h1')).toContainText('Nie odnaleziono strony')
  })

  test('Free: logout clears session and denies access', async ({ page }) => {
    await page.goto('/account')
    await page.locator('button:has-text("Wyloguj")').click()
    await expect(page).toHaveURL(/\/login|\/$/)

    // po logout /account i /history są niedostępne
    await page.goto('/account')
    await expect(page).toHaveURL(/\/login/)

    await page.goto('/history')
    await expect(page.locator('text=Zapisuj i śledź historię swoich audytów').first()).toBeVisible()
  })
})
